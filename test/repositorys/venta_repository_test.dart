...
void main() {
  ...
  group('VentaRepository Tests', () {
    ...
    late ProdutoRepository productoRepo;
    ...
    Future<int> _crearUnidad() async {
      return await unidadRepo.create(UnidadMedida(nombre: 'Gramos'));
    }

    Future<int> _crearInsumo(int unidadId) async {
      return await insumoRepo.create(Insumos(nombre: 'Café en Grano', idUnidad: unidadId));
    }

    Future<int> _crearProducto(List<InsumoProducto> insumos) async {
      final productoId = await productoRepo.create(Producto(nombre: 'Café Americano', precioVenta: 2.50));
    ...
    Future<int> _crearCliente() async {
      return await clienteRepo.create(Cliente(nombre: 'Juan', apellido: 'Perez', telefono: '123456789'));
    }

    Future<Venta> _crearVentaCompleta() async {
      final clienteId = await _crearCliente();
      final unidadId = await _crearUnidad();
      final insumoId = await _crearInsumo(unidadId);
      final productoId = await _crearProducto([
        InsumoProducto(idInsumo: insumoId, idProducto: 0, cantidadRequerida: 20) // idProducto es dummy
      ]);
    ...
    setUp(() async {
      ...
      database = await openDatabase(
        path,
        version: 2,
        onCreate: (db, version) async => await DatabaseHelper().testOnCreate(db),
      ...
      productoRepo = ProdutoRepository(databaseHelper, insumoRepo);
    ...
    test('getFullVenta loads all relationships correctly', () async {
      ...
      expect(ventaCompleta.detallesVenta.first.producto!.nombre, 'Café Americano');
    });

    test('getVentasByCliente returns all ventas for a specific client', () async {
      ...
      await ventaRepo.createWithDetails(venta1); // Crear otra venta para el mismo cliente

      // ACT: Obtener las ventas por ID de cliente
      final ventasCliente = await ventaRepo.getVentasByCliente(venta1.idCliente);
    ...
    test('createWithDetails with empty detalles does not fail', () async {
      ...
      final clienteId = await _crearCliente();
      final venta = Venta(idCliente: clienteId, fecha: DateTime.now(), detallesVenta: []);
    ...
    test('getVentasByCliente returns empty list for client with no ventas', () async {
    ...
    test('Performance: Bulk venta creation with batch', () async {
      ...
      final unidadId = await _crearUnidad();
      final insumoId = await _crearInsumo(unidadId);
      final productoId = await _crearProducto([InsumoProducto(idInsumo: insumoId, idProducto: 0, cantidadRequerida: 1)]);
      ...
      for (int i = 0; i < recordCount; i++) {
        batch.insert('Venta', {'id_cliente': clienteId, 'fecha': DateTime.now().toIso8601String(), 'pagado': 0});
      }
      ...
      stopwatch.stop();
      print('Creación de $recordCount ventas en batch: ${stopwatch.elapsedMilliseconds}ms');
